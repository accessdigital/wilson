...
((Drupal, once) => {
  ...
  Drupal.behaviors.tasksInToolbar = {
    attach(context) {
      ...
      const localTasksEl = once("localTasks", ".local-tasks-in-toolbar > ul", context).shift();
      ...
      if (toolbarEl && localTasksEl) {
        ...
        // Simulate a click on the new tab so that it's initially the active tab.
        // Added a delay so that this happens after the toolbar JS is initialised.
        window.setTimeout(() => {
          if (!toolbarLink.classList.contains('is-active')) {
            toolbarLink.click();
          }
        }, 100);
      }
